...
typedef enum : NSUInteger {
    kFRFileUnitStatus_FileNonReady,
    kFRFileUnitStatus_Stop,
    kFRFileUnitStatus_Pause,
    kFRFileUnitStatus_Play,
} kFRFileUnitStatus;

@interface FRFileUnit () {
    AudioFileID fileID;
...
@implementation FRFileUnit

-(id)init {
    if (self = [super initWithType:kFRAudioUnitType_File]) {
...
-(void)playWithTime:(float)time {
    switch (status) {
        case kFRFileUnitStatus_Play:
        case kFRFileUnitStatus_Pause:
            return;
            break;
            
        case kFRFileUnitStatus_Stop:
...
-(BOOL)isPlay {
    if (status == kFRFileUnitStatus_Play)
        return  YES;
    else
        return NO;
}
